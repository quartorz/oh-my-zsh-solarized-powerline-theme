...
if [ $ZSH_POWERLINE_SHOW_USER = true ] || [ $ZSH_POWERLINE_SHOW_IP = true ]; then
	if [ $ZSH_POWERLINE_SHOW_TIME = true ]; then
		PROMPT="${PROMPT} ${FG_COLOR_BASE3}${BG_COLOR_CYAN}${ARROW_SYMBOL}"
	else
...
# datetime
if [ $ZSH_POWERLINE_SHOW_TIME = true ]; then
	PROMPT="${PROMPT}${FG_COLOR_BASE03}${BG_COLOR_CYAN}${PADDING}${ZSH_TIME}"
	PROMPT="${PROMPT} ${FG_COLOR_CYAN}${BG_COLOR_BASE02}${ARROW_SYMBOL}"
	PADDING=' '
fi
...
if [ $ZSH_POWERLINE_SINGLE_LINE = false ]; then
	PROMPT="${PROMPT} %E
${BG_COLOR_BASE03} ${RESET}${FG_COLOR_BASE03}${ARROW_SYMBOL}"
else
